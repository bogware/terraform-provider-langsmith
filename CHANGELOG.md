...
* Self-hosted instances: resources that use the platform API family (`langsmith_evaluator`, `langsmith_tool`, `langsmith_gateway_policy`, `langsmith_mcp_vendor_settings`, `langsmith_access_policy`, and others on `/v1/platform/...`, plus `/v2/sandboxes/...`, `/commits/...`, and `/workspaces/current/...` paths) were broken on self-hosted LangSmith. Self-hosted mounts the entire API under a `/api` path prefix, while Cloud serves it at the root of the `api.` subdomain, so these paths fell through to the frontend web app (HTTP 200 with SPA HTML, or 405) instead of reaching the API. Set the new provider attribute `self_hosted = true` (or `LANGSMITH_SELF_HOSTED=true`) and the provider adds the `/api` prefix where needed. Cloud behavior is unchanged (`self_hosted` defaults to `false`). ([#64](https://github.com/bogware/terraform-provider-langsmith/issues/64))
