...
* `langsmith_prompt` could not commit a changed `manifest` on repos with more than one head — typically a repo adopted into Terraform alongside a pre-existing commit lineage. The commit payload carried only `manifest`, leaving the API to infer where to attach the new commit, and that inference fails on an ambiguous graph: every apply died with `409 Parent commit validation failed` and the live commit never advanced, while the same edit committed fine through the LangSmith UI. The provider now sends `parent_commit`, pinned to the head recorded in state, exactly as `langsmith_hub_directory` already did. A conflict on that parent means the prompt was committed to outside Terraform since the last refresh, so a 409 whose body reports a parent-commit conflict now carries a diagnostic saying so instead of the raw API body. ([#81](https://github.com/bogware/terraform-provider-langsmith/issues/81))
* `langsmith_prompt` could silently lose applied metadata when an update changed both the repo settings and the manifest. The `PATCH` lands first, so if the follow-up commit then failed, the resource returned without writing state — and terraform-plugin-framework seeds the update response from the *prior* state, so Terraform persisted the pre-update description, tags and `tag_value_ids` even though the API already had the new ones. State stayed wrong until something forced a refresh, and `tag_value_ids` is never re-read, so for that attribute it stayed wrong indefinitely. The already-applied metadata is now persisted on the failure path, alongside the unchanged manifest and commit hash. This is the `Update` counterpart to the partial-state fix `Create` received in 1.1.0.
...
* `langsmith_run_rule` showed a permanent diff on `evaluators` / `code_evaluators`. The API expands them on the way in (a code evaluator submitted as `{"code": "..."}` comes back with `"language": "python"`), and `Read` wrote the expanded copy into state. The configured form is now kept whenever the response merely adds to it; a genuine server-side change still surfaces.
* `langsmith_project` failed `ImportStateVerify` on `start_time`. The create response omits the time zone where the read response includes it, so the same instant was stored two ways. Timestamps are now canonicalised to UTC, while a configured value that denotes the same instant keeps its original spelling.

* `langsmith_data_plane` now deletes the data plane on `terraform destroy`. It previously removed the resource from state and warned that the API had no delete endpoint, which is no longer true — `DELETE /orgs/current/data-planes/{id}` exists, so destroying the resource silently left the data plane running. If a deployment does not expose the endpoint (HTTP 404 or 405) the previous behavior is kept: the resource leaves state and a warning explains that the data plane must be deprovisioned through LangSmith support.
* Self-hosted instances: `langsmith_prompt`, `langsmith_prompt_tag`, the `langsmith_prompt_commit` data source, `langsmith_sandbox_registry` and the `langsmith_sandbox_registries` data source were still broken on self-hosted. They call root-mounted paths (`/commits/...`, `/v2/sandboxes/registries`) that the 1.1.0 `self_hosted` rewrite deliberately did not cover, so those requests fell through to the frontend web app instead of reaching the API. All of them now use the canonical `/api`-prefixed form (`/api/v1/commits/...`, `/api/v2/sandboxes/registries`), which LangSmith Cloud serves identically, so they work on both deployment types with no extra configuration. ([#64](https://github.com/bogware/terraform-provider-langsmith/issues/64), [#77](https://github.com/bogware/terraform-provider-langsmith/issues/77))
...
* New resource `langsmith_optimization_job`, running a prompt optimization job against a Hub repo, paired with a `langsmith_optimization_job_logs` data source for its log stream. A job is a unit of work rather than steady state: the server advances `status` on its own, and `algorithm` / `config` cannot be changed once submitted, so editing either forces a new job. Imported as `<owner>/<repo>/<job_id>`. Acceptance tests are opt-in behind `LANGSMITH_TEST_OPTIMIZATION` because a run incurs model spend.
* New resource `langsmith_hub_directory`, managing the file contents of a directory-style Hub repository. Each apply that changes `files` writes a commit, with the known head sent as the parent so a write that raced with an out-of-band change is rejected rather than silently overwriting it. Imported as `<owner>/<repo>`.
* New data source `langsmith_info_health`, reporting instance health (currently ClickHouse disk headroom) — most useful on self-hosted, where storage is the operator's problem.
* `langsmith_oauth_client` gained a `rotate_secret` trigger attribute: change its value and the next apply issues a fresh `client_secret` while leaving `client_id` and every other setting intact. The value is never sent to LangSmith; only the fact that it changed matters.
